...
void sort_function(void *data, size_t size, size_t data_size)
{
  ...
  for (size_t i = 0; i < size - 1; i++)
  {
    ...
    {
      char *currentJ = (char *)data + (j * data_size);
      char *currentJ1 = (char *)data + ((j + 1) * data_size);
      int compare = memcmp(currentJ, currentJ1, data_size);
      if (compare > 0)
      {
        memcpy(tmp, currentJ, data_size);
        memcpy(currentJ, currentJ1, data_size);
        memcpy(currentJ1, tmp, data_size);
      }
    }
  }
  free(tmp);
...
void pipeline_register(Pipeline *pipeline, P_FLAG flags)
{
  ...
  {
    if (flags & P_MAP)
      pipeline->operations[i] = map_function;
    else
      pipeline->operations[i] = NULL;

    if (flags & P_REDUCE)
      pipeline->operations[i] = reduce_function;
    else
      pipeline->operations[i] = NULL;

    if (flags & P_SORT)
      pipeline->operations[i] = sort_function;
    else
      pipeline->operations[i] = NULL;

    if (flags & P_FILTER)
      pipeline->operations[i] = filter_function;
    else
      pipeline->operations[i] = NULL;
  }
}
